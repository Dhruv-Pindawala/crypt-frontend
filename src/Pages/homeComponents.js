...
export const UserMain = (props) => {
    ...
    return (
        <div className={`flex align-center justify-between userMain ${props.clickable ? "clickable" : ""}`} onClick={() => props.clickable && props.onClick()}>
...
export const UserAvatar = (props) => {
    return (
        <div className={`userAvatar ${props.isV2 ? "version2":""}`}>
            <div className = 'imageCon' style={{backgroundImage: `url("${props.profilePicture}")`,}} />
...
export const ProfileModal = (props) => {
  const [profileData, setProfileData] = useState({...props.userDetail, user_id: props.userDetail.user.id,});
  ...
  const submit = async (e) => {
    ...
    const profile = await axiosHandler({method,url,data: profileData,token,}).catch((e) => alert(errorHandler(e)));
    setSubmitted(false);
    if (profile) {
      props.setClosable();
  ...
  useEffect(() => {
    if(props.visible){
      setProfileData({...props.userDetail,user_id: props.userDetail.user.id,})
        ...
        <form className="content" onSubmit={submit}>
          <div className="inner">
            <div className="leftHook">
              <div className="imageCon" style={{backgroundImage: `url(${pp})`,}}/>
            ...
            <div className="dataInput">
              <label>
                ...
                <input name="first_name" value={profileData.first_name} onChange={onChange} disabled={props.view} required />
              </label>
              <label>
                <span>Last name</span>
                <input name="last_name" value={profileData.last_name} onChange={onChange} disabled={props.view} required />
              </label>
              <label>
                <span>Caption</span>
                <input name="caption" value={profileData.caption} onChange={onChange} disabled={props.view} required />
              </label>
              <label>
                <span>About</span>
                <textarea name="about" value={profileData.about} onChange={onChange} disabled={props.view} required />
              </label>
            </div>
          </div>
          {
